...
export default function FilterBar() {
  ...
  // Helper to read params safely (works even when sp is null at build time)
  ...
  const setQ = (k: string, v: string) => {
    const current = sp ? new URLSearchParams(sp.toString()) : new URLSearchParams()
    current.set(k, v)
    if (k === 'scope') {
      current.set('code', v === 'store' ? 'BANYAN' : v === 'group' ? 'PRIME' : 'ALL')
    }
